...
void loads(Game *p_game){
	...
	p_game->font_pequena_2 = al_load_ttf_font("assets/fonts/arial.ttf", 10, 0);  
	...
	p_game->sprites.background_final = al_load_bitmap("assets/sprites/background_final.png");
	...
	p_game->sprites.object = al_load_bitmap("assets/sprites/object.png");
	...
	p_game->audio.game_over =al_load_sample("assets/audios/game_over.wav");
	p_game->audio.victory =al_load_sample("assets/audios/victory.wav");

	// -- MUSICA -- 2

	p_game->audio.musica_jogo = al_load_audio_stream("assets/audios/musica_jogo_2.ogg", 4, 2048);
	p_game->audio.musica_menu = al_load_audio_stream("assets/audios/musica_menu_2.ogg", 4, 2048);

	// ---- configuracao de som ----
	al_set_audio_stream_gain(p_game->audio.musica_menu, 0.025);
	al_set_audio_stream_gain(p_game->audio.musica_jogo, 0.1);
...
void destroys(Game *p_game){
    ...
    al_destroy_font(p_game->font_grande);
	al_destroy_font(p_game->font);
	al_destroy_font(p_game->font_pequena);
	al_destroy_font(p_game->font_pequena_2);
	...
	al_destroy_bitmap(p_game->sprites.background_final);
	...
	al_destroy_bitmap(p_game->sprites.object);
	...
	al_destroy_sample(p_game->audio.game_over);
	al_destroy_sample(p_game->audio.victory);
